...
| hybrid map保存 | 近傍raw deskew済み点群と遠方feature点群をlocal frameで外部map builderへ渡す | `CloudInfo.msg`, `imageProjection.cpp`, `mapOptmization.cpp`, `pcd_map_saver_node.cpp` |
...
| `hybridRegisteredCloudEnabled` | 地図保存用hybrid local点群のpublish有効化 | `true` | `true` | `true` |
| `hybridRegisteredCloudRawNearRange` | hybrid点群でraw deskew済み近傍点を優先する距離 | `2.0` | `3.0` | `3.0` |
| `hybridRegisteredCloudRawNearLeafSize` | 近傍raw deskew済み点群のVoxelGrid leaf size。`0.0`で無効 | `0.01` | `0.01` | `0.01` |
| `hybridRegisteredCloudTopic` | hybrid local点群のpublish topic | `/lio_sam/mapping/cloud_registered_hybrid` | 同左 | 同左 |
...
| `processingTimeLogIntervalSec` | imageProjectionとhybrid生成の平均処理時間ログ周期。`0.0`で無効 | `5.0` | `5.0` | `5.0` |
...
| `publishCloudRegisteredRaw` | `/lio_sam/mapping/cloud_registered_raw` のpublish | `false` | `false` | `false` |
...
`CloudInfo.msg` に `cloud_deskewed_raw_near` を追加した。`imageProjection.cpp` では `downsampleRate` 判定より前に `hybridRegisteredCloudRawNearRange` 内の点をdeskewし、SLAM用間引きとは別に近傍raw点群を保持する。

近傍raw点群は密度が高くなりやすいため、`hybridRegisteredCloudRawNearLeafSize` のVoxelGridをpublish前に適用する。既定は `0.01m` で、`0.0` を指定すると無効化できる。
...
`mapOptmization.cpp` では `cloud_registered_hybrid` 生成時にraw近傍点群のROS変換、遠方feature合成、publishの平均処理時間を `Hybrid cloud timing` として出力する。遠方feature判定は平方距離で行い、sqrt計算を避ける。
...
この点群はSLAM最適化には使わず、`mapOptmization.cpp` で `cloud_registered_hybrid` をlocal frameでpublishするために使う。
...
| 項目 | 既定値 |
|---|---|
| `pointCloudTopic` | `/fused/livox/lidar` |
...
| node | 既定 | 目的 |
|---|---|---|
| `multi_lidar_pointcloud_fusion_node` | 有効 | 複数LiDARのCustomMsg fusionとLIO-SAM入力生成 |
| `pcd_map_saver_node` | 無効 | 全local scan点群をkeyframe区間submapへ合成し、最新path poseで再配置してPCD保存する |
...
| LIO-SAM LiDAR topic | `/fused/livox/lidar` |
...
|---|---|---|---|
| Livox driver / sim adapter | `/lidar1/livox/lidar` | `multi_lidar_pointcloud_fusion_node` | 基準LiDARのLivox `CustomMsg` scan |
| `multi_lidar_pointcloud_fusion_node` | `/fused/livox/lidar` | `lio_sam_imageProjection` | fusion後のLivox `CustomMsg` LiDAR scan |
...
| `lio_sam_mapOptimization` | `lio_sam/mapping/cloud_registered_hybrid` | `pcd_map_saver_node` | local frameの近傍raw + 遠方feature点群 |
