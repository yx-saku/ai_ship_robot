...
| map保存軽量化 | raw登録点群をmap saverへ渡し、localization PCDと2.5D elevation submapを保存する | `mapOptmization.cpp`, `map_saver_node.cpp` |
...
| `publishCloudRegisteredRaw` | `/lio_sam/mapping/cloud_registered_raw` のpublish。map saver有効時はlaunchでtrueへ上書き | `false` | `false` | `false` |
...
`mapOptmization.cpp` は、map saver有効時にscan local frameのraw登録点群を `/lio_sam/mapping/cloud_registered_raw` としてpublishする。これはROS driver直後の未処理点群ではなく、multi-LiDARを基準LiDAR frameへ合成し、deskew済みでmapOptimizationから出る点群である。

`map_saver_node` はraw登録点群を未確定submap分だけ保持し、keyframe区間が確定した時点でlocalization用の粗いsubmapと2.5D elevation submapへ変換して元raw点群を破棄する。

`lio_sam.launch.py` では map saver 有効時だけ `publishCloudRegisteredRaw` をtrueに上書きし、map saver入力topicの既定値を `/lio_sam/mapping/cloud_registered_raw` にする。

localization用PCDは保存時に最新path poseでtarget frameへ再配置し、`localization_map.pcd` としてbinary PCDで保存する。

2.5D elevation submapは、submap確定時のpath/map frame zとanchor yaw-local XYを使い、CSVとmanifest YAMLとして保存する。

`mapping/odometry` と `mapping/path` は、raw点群のpose同期と保存時のlocalization submap再配置に使う。

旧hybrid点群生成は通常のmap保存では使わず、既定で無効化する。
...
| `map_saver_node` | 無効 | raw登録点群からlocalization PCDと2.5D elevation submapを保存する |
...
| `lio_sam_mapOptimization` | `lio_sam/mapping/cloud_registered_raw` | `map_saver_node` | scan local frameのdeskew済みraw登録点群 |
| `lio_sam_mapOptimization` | `lio_sam/mapping/odometry` | `map_saver_node` | 各scan点群をsubmapへ変換するためのscan pose |
| `lio_sam_mapOptimization` | `lio_sam/mapping/path` | `map_saver_node` | keyframe pose列。loop closure後は補正済みpose列として再publishされる |
