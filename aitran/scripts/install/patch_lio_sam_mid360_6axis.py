...
def replace_once(text: str, old: str, new: str, path: Path, label: str) -> str:
    ...
    if label == "timer paced LiDAR callback" and "void processCloudQueue()" in text and "processCloudQueue();\n    }\n\n    void processCloudQueue" not in text:
        return text
    if label == "single scan cloud queue processing" and "1回のtimerで1scanだけ処理" in text:
        return text
    if old not in text:
...
""",
        path,
        "single scan cloud queue processing",
    )
    text = replace_once(
        text,
...
""",
        path,
        "internal incremental odometry publisher member",
    )
...
""",
        path,
        "internal incremental odometry publisher",
    )
    text = replace_once(
        text,
...
""",
        path,
        "initial LiDAR output origin before first keyframe",
    )
    text = replace_all(text, "pose_stamped.header.frame_id = odometryFrame;", "pose_stamped.header.frame_id = lidarInitFrame;", path, "path frame")
    text = replace_all(text, "laserOdometryROS.header.frame_id = odometryFrame;", "laserOdometryROS.header.frame_id = lidarInitFrame;", path, "global odometry parent frame")
    text = replace_all(text, 'laserOdometryROS.child_frame_id = "odom_mapping";', "laserOdometryROS.child_frame_id = odometryFrame;", path, "global odometry child frame")
    text = replace_all(text, "laserOdometryROS.child_frame_id = lidarFrame;", "laserOdometryROS.child_frame_id = odometryFrame;", path, "patched global odometry child frame")
    text = replace_all(
        text,
    ...
    text = replace_all(text, 'trans_odom_to_lidar.child_frame_id = "lidar_link";', "trans_odom_to_lidar.child_frame_id = odometryFrame;", path, "TF child frame")
    text = replace_all(text, "trans_odom_to_lidar.child_frame_id = lidarFrame;", "trans_odom_to_lidar.child_frame_id = odometryFrame;", path, "patched TF child frame")
    old_suppressed_lidar_tf = (
        "        // AI_SHIP_ROBOT_MID360_6AXIS_PATCH: left_lidar_link is a URDF static child of base_link.\n"
        "        // Publishing odom->left_lidar_link here conflicts with robot_state_publisher.\n"
        "        // imuPreintegration publishes odom->baselinkFrame after applying lidar->base instead.\n"
        "        // br->sendTransform(trans_odom_to_lidar);"
    )
    new_lidar_odometry_tf = (
        "        // AI_SHIP_ROBOT_MID360_6AXIS_PATCH: publish LiDAR odometry as base_lidar_init->odom.\n"
        "        br->sendTransform(trans_odom_to_lidar);"
    )
    if new_lidar_odometry_tf not in text:
        if old_suppressed_lidar_tf in text:
            text = text.replace(old_suppressed_lidar_tf, new_lidar_odometry_tf, 1)
        else:
            text = replace_all(
                text,
                "        br->sendTransform(trans_odom_to_lidar);",
                new_lidar_odometry_tf,
                path,
                "base_lidar_init odometry TF publish",
            )
    text = replace_all(text, "laserOdomIncremental.header.frame_id = odometryFrame;", "laserOdomIncremental.header.frame_id = lidarInitFrame;", path, "incremental odometry parent frame")
    text = replace_all(text, 'laserOdomIncremental.child_frame_id = "odom_mapping";', "laserOdomIncremental.child_frame_id = odometryFrame;", path, "incremental odometry child frame")
    text = replace_all(text, "laserOdomIncremental.child_frame_id = lidarFrame;", "laserOdomIncremental.child_frame_id = odometryFrame;", path, "patched incremental odometry child frame")
...
""",
        path,
        "relative LiDAR output affine helper",
    )
...
""",
        ...
        """    void updatePath(const PointTypePose& pose_in)
    {
        ...
        pose_stamped.header.frame_id = lidarInitFrame;
...
""",
        path,
        "relative LiDAR global odometry pose",
    )
    text = replace_once(
        text,
        """        Eigen::Affine3f outputLidarAffine = relativeLidarOutputAffine(trans2Affine3f(transformTobeMapped));
""",
        """        Eigen::Affine3f internalLidarAffine = trans2Affine3f(transformTobeMapped);
        Eigen::Affine3f outputLidarAffine = relativeLidarOutputAffine(internalLidarAffine);
""",
        path,
        "internal LiDAR affine before public output",
    )
    text = replace_once(
        text,
        """        quat_tf.setRPY(transformTobeMapped[0], transformTobeMapped[1], transformTobeMapped[2]);
...
""",
        path,
        "relative LiDAR incremental odometry pose",
    )
    text = replace_once(
        text,
...
""",
        path,
        "internal incremental odometry state",
    )
    text = replace_once(
        text,
...
""",
        path,
        "initialize internal incremental odometry",
    )
    text = replace_once(
        text,
        """            laserOdomIncremental = laserOdometryROS;
...
""",
        path,
        "initialize patched internal incremental odometry",
    )
    text = replace_once(
        text,
        """            Eigen::Affine3f outputIncreOdomAffine = relativeLidarOutputAffine(increOdomAffine);
...
""",
        path,
        "publish internal incremental odometry before public relative odometry",
    )
    text = replace_once(
        text,
        """            if (cloudInfo.imu_available == true)
...
""",
        path,
        "patched incremental IMU blend suppression",
    )
    text = replace_once(
        text,
        """        pubLaserOdometryIncremental->publish(laserOdomIncremental);
""",
        """        pubLaserOdometryIncrementalInternal->publish(laserOdomIncrementalInternal);
        pubLaserOdometryIncremental->publish(laserOdomIncremental);
""",
        path,
        "publish internal incremental odometry topic",
    )
    text = replace_all(text, "globalPath.header.frame_id = odometryFrame;", "globalPath.header.frame_id = lidarInitFrame;", path, "path header frame")
    text = replace_all(text, 'double imuWeight = 0.1;', 'double imuWeight = imuRPYWeight;', path, "incremental imu RPY weight")
    text = strip_trailing_whitespace(text)
    return write_text_if_changed(path, original, text)
...
def patch_imu_preintegration(lio_sam_dir: Path) -> bool:
    ...
    text = replace_all(
        ...
        "            \"lio_sam/mapping/odometry_incremental_internal\", qos,",
        path,
        "IMU preintegration internal odometry subscription",
    )
    text = replace_all(
        text,
        "        tfBroadcaster->sendTransform(ts);",
        "        // AI_SHIP_ROBOT_MID360_6AXIS_PATCH: odom->base is a static extrinsic TF from launch.\n"
        "        // Publishing it here as dynamic TF would make base_footprint drift twice.\n"
        "        // tfBroadcaster->sendTransform(ts);",
        path,
        "TransformFusion dynamic base TF suppression",
    )
    text = strip_trailing_whitespace(text)
    return write_text_if_changed(path, original, text)
