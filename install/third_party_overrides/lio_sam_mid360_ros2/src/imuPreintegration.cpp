...
class TransformFusion : public ParamServer {
   public:
    ...
    void imuOdometryHandler(const nav_msgs::msg::Odometry::SharedPtr odomMsg) {
        ...
        // AI_SHIP_ROBOT_BEGIN: LiDAR補正時刻と同時刻のsampleだけが残ってもfront/back参照を壊さない。
        ...
        // AI_SHIP_ROBOT_BEGIN: base TFはproject側static TFへ集約し、TransformFusionから重複publishしない。
        // base_footprintへのTFはproject側static TFに任せ、TransformFusionからの重複publishを避ける。
        (void)ts;
        // AI_SHIP_ROBOT_END
   ...
   public:
    ...
    void odometryHandler(const nav_msgs::msg::Odometry::SharedPtr odomMsg) {
        ...
        if (systemInitialized == false) {
            ...
            // AI_SHIP_ROBOT_BEGIN: 初回optimizer失敗時はprocessを落とさず次の補正を待つ。
            // optimize once
            try {
        ...
        // AI_SHIP_ROBOT_BEGIN: 初回sampleのdt、ゼロduration補正、bias noiseをIMU間隔に合わせて扱う。
        ...
        while (!imuQueOpt.empty()) {
            ...
            if (imuTime < currentCorrectionTime - delta_t) {
                double dt = (lastImuT_opt < 0) ? fallbackImuDeltaTime() : (imuTime - lastImuT_opt);
        ...
        graphFactors.add(gtsam::BetweenFactor<gtsam::imuBias::ConstantBias>(
            ...
            gtsam::noiseModel::Diagonal::Sigmas(sqrt(integratedImuDuration) * noiseModelBetweenBias)));
        // AI_SHIP_ROBOT_END
        // add pose factor
        ...
        // AI_SHIP_ROBOT_BEGIN: optimizer例外を捕捉してIMU preintegrationをreset継続する。
        // optimize
        try {
            optimizer.update(graphFactors, graphValues);
            optimizer.update();
        ...
        if (!imuQueImu.empty()) {
            ...
            for (int i = 0; i < (int)imuQueImu.size(); ++i) {
                ...
                double imuTime = stamp2Sec(thisImu->header.stamp);
                double dt = (lastImuQT < 0) ? fallbackImuDeltaTime() : (imuTime - lastImuQT);
    ...
    void imuHandler(const sensor_msgs::msg::Imu::SharedPtr imu_raw) {
        ...
        // AI_SHIP_ROBOT_BEGIN: 初期化中のIMUもqueueへ保持し、先に届いた初回mapping補正を捨てない。
        ...
        double imuTime = stamp2Sec(thisImu.header.stamp);
        double dt = (lastImuT_imu < 0) ? fallbackImuDeltaTime() : (imuTime - lastImuT_imu);
