...
usage() {
  cat <<'EOF'
Usage: bash scripts/install/setup.sh
...
事前に `bash scripts/install/install.sh` と `bash scripts/install/install_third_party.sh` を実行してください。
EOF
}
...
require_third_party_underlay() {
  if [[ ! -f "${THIRD_PARTY_UNDERLAY_SETUP}" ]]; then
    ...
    echo "Run bash scripts/install/install_third_party.sh first." >&2
    exit 1
  fi
}
...
# This file is managed by scripts/install/setup.sh.
...
if [ -f "/opt/ros/\${ROS_DISTRO}/setup.bash" ]; then
  ...
  if [ -f "\${AI_SHIP_ROBOT_WORKSPACE}/ros2_ws_sim/install/setup.bash" ]; then
