...
ROS_SIM_WS="${WORKSPACE_ROOT}/ros2_ws_sim"
...
require_ros2() {
  if [[ ! -f "/opt/ros/${ROS_DISTRO}/setup.bash" ]]; then
    echo "Missing /opt/ros/${ROS_DISTRO}/setup.bash. Run bash scripts/install/install.sh first." >&2
    exit 1
  fi
}
...
require_third_party_underlay() {
  if [[ ! -f "${THIRD_PARTY_UNDERLAY_SETUP}" ]]; then
    ...
    echo "Run bash scripts/install/install_third_party.sh && bash scripts/install/sim/install_third_party.sh first." >&2
    exit 1
  fi
}
...
source_third_party_underlay() {
  # simulation pluginを含むsystem underlayを先に読み込み、ros2_ws_simの依存先にする。
  set +u
...
require_simulation_underlay_package() {
  if ! ros2 pkg prefix ros2_livox_simulation >/dev/null 2>&1; then
    ...
    echo "Run bash scripts/install/sim/install_third_party.sh first." >&2
    exit 1
  fi
}
...
# This file is managed by scripts/install/sim/setup.sh.
...
if [ -f "/opt/ros/\${ROS_DISTRO}/setup.bash" ]; then
  ...
  _ai_ship_robot_source_overlay_if_current "\${AI_SHIP_ROBOT_WORKSPACE}/ros2_ws_sim/install/setup.bash"
...
build_project_workspace() {
  ...
  # simulation workspaceは専用の ros2_ws_sim だけをbuildし、本番導線と成果物を分離する。
