...
usage() {
  cat <<'EOF'
Usage: bash scripts/install/install.sh
...
install_runtime_packages() {
  local packages=(build-essential cmake git libeigen3-dev libboost-all-dev libgomp1 libpcl-dev libtbb-dev python3-colcon-common-extensions python3-rosdep python3-vcstool sudo "ros-${ROS_DISTRO}-ament-cmake" "ros-${ROS_DISTRO}-ament-cmake-auto" "ros-${ROS_DISTRO}-diagnostic-msgs" "ros-${ROS_DISTRO}-geometry-msgs" "ros-${ROS_DISTRO}-launch" "ros-${ROS_DISTRO}-launch-ros" "ros-${ROS_DISTRO}-message-filters" "ros-${ROS_DISTRO}-nav-msgs" "ros-${ROS_DISTRO}-pcl-conversions" "ros-${ROS_DISTRO}-pcl-ros" "ros-${ROS_DISTRO}-robot-state-publisher" "ros-${ROS_DISTRO}-ros-base" "ros-${ROS_DISTRO}-ros2launch" "ros-${ROS_DISTRO}-ros2run" "ros-${ROS_DISTRO}-ros2service" "ros-${ROS_DISTRO}-ros2topic" "ros-${ROS_DISTRO}-rosbag2" "ros-${ROS_DISTRO}-sensor-msgs" "ros-${ROS_DISTRO}-std-msgs" "ros-${ROS_DISTRO}-tf2-ros" "ros-${ROS_DISTRO}-tf2-sensor-msgs" "ros-${ROS_DISTRO}-xacro")
  # 実機導線で必要なROS基盤とGLIM依存をまとめて導入し、workspace無依存のsystem導線にする。
