...
usage() {
  cat <<'EOF'
Usage: bash scripts/install/install_third_party.sh

実機・simulation共通の third_party を system 側へ導入します。
- Livox-SDK2 を /opt/ai_ship_robot/vendor/livox_sdk2 に導入
- livox_ros_driver2 / GTSAM / gtsam_points / GLIM / glim_ros2 を ROS underlay として導入
...
patch_gtsam_points_repo() {
  ...
  # Ubuntu 22.04標準Boostでのビルド失敗を避けるため、constexpr指定だけを外す最小patchを当てる。
...
selected_common_ros_paths() {
  printf '%s\n' "${LIVOX_DRIVER_DIR}" "${GTSAM_DIR}" "${GTSAM_POINTS_DIR}" "${GLIM_DIR}" "${GLIM_ROS2_DIR}"
}
...
build_common_underlay_workspace() {
  if [[ ! -d "${LIVOX_DRIVER_DIR}" || ! -d "${GTSAM_DIR}" || ! -d "${GTSAM_POINTS_DIR}" || ! -d "${GLIM_DIR}" || ! -d "${GLIM_ROS2_DIR}" ]]; then
    echo "Missing common third-party repositories under ${THIRD_PARTY_SRC_DIR}" >&2
    return 1
  fi

  # 実機driverとGLIM系underlayを同じinstall-baseへ積み、プロジェクトworkspaceの下地にする。
