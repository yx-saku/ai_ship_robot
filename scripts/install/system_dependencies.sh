...
collect_missing_packages() {
  ...
  # dpkg-queryをまとめて呼び、再実行時やDocker layer cache復帰時の細かいプロセス起動を減らす。
...
write_apt_network_config() {
  ...
  # IPv4固定、retry、timeoutを1箇所に集約し、Docker buildと直接実行の挙動を揃える。
  {
...
configure_ubuntu_apt_mirrors() {
  # mirror指定は任意にし、未指定ならbase imageの既定sourcesを維持する。
...
enable_apt_component_in_sources_list() {
  ...
  # software-properties-commonを入れず、deb行へ必要componentだけを直接追加して依存導入を軽くする。
...
ensure_locales_generated() {
  ...
  # 生成済みlocaleは再生成せず、system依存scriptの再実行を軽くする。
...
ensure_rosdep_initialized() {
  if command -v rosdep >/dev/null 2>&1 && [[ ! -f /etc/ros/rosdep/sources.list.d/20-default.list ]]; then
...
configure_ros2_apt_repository() {
  ...
  if [[ ! -f "${ros_keyring_path}" ]]; then
    curl -fsSL https://raw.githubusercontent.com/ros/rosdistro/master/ros.key | ${SUDO} tee "${ros_keyring_path}" >/dev/null
  fi
